...
for s = 1:length(SIDs)
    ...
    for i = 1:segNum
 	...
 	if exist([InPath '/' runname '_Atlas_hp200_s4.dtseries.nii'])~=0
	       OutPath = ['/zfs/musc/david/HCP4variability/data/' sub '/' runname];
        mkdir(OutPath)	
        cifti = cifti_read([InPath '/' runname '_Atlas_hp200_s4.dtseries.nii']);        
       ...
       ciftisave(cifti,[OutPath '/'  runname '_Atlas_hp200_s4_bpss_12mr.dtseries.nii']);
       ...
       cifti_write(cifti,[OutPath '/' runname '_Atlas_hp200_s4_bpss_12mr_gsr.dtseries.nii']);
	end
    end
end